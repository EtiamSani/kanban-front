const listModule = {
    ...
    makeListInDOM: function (listeObjet) {
        ...
        //Recuperation du container de box
        const cardsContainer = copieListe.querySelector('.panel-block')

        //on rend le container de box drag n droppable
        Sortable.create(cardsContainer, {
          draggable:'.box',
          onEnd:listModule.validerChangementDePositionCard,
          group:'wololo'
        })
  ...
  getListsFromApi: async function() {
    ...
    //Declarer une instance de Sortable

    Sortable.create(listsContainer, {
      //Les elements glissable sont ceux qui ont la classe css .panel
      draggable:'.panel',
      onEnd:listModule.validerChangementDePosition
    })
  ...
  //Fonction executée quand une carte change de position
  validerChangementDePositionCard : async function(event) {

    console.log(event.to)

    const idDeLaListeQuiReceptionne = event.to.closest('.panel').dataset.listId

    const boxes = event.to.querySelectorAll('.box')
    ...
    for(const box of boxes)
    {
      ...
      formData.set("listId",idDeLaListeQuiReceptionne)

      position++

      console.log(app.base_url + "/cards/" + box.dataset.cardId)

      await fetch(app.base_url + "/cards/" + box.dataset.cardId,{
        method:'PUT',
        body:formData
      })
  ...
  //Fonction executée quand une liste change de position
  validerChangementDePosition: function(event) {

    const lesListes = document.querySelectorAll('.panel')

    let position = 0

    for(const liste of lesListes)
    {
        const formData = new FormData()
        formData.set("position",position)
        ...
        fetch(app.base_url + "/lists/" + liste.dataset.listId,{
          method:'PUT',
          body:formData
        })
    }
  ...
  validerEditionFormulaire: async function(event)
  {
    ...
    await fetch(app.base_url + "/lists/" +  formData.get("id"), 
    {
      method:"PUT",
      body:formData
    })
